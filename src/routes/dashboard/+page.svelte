<script lang="ts">
  ...
  async function notifyDashboardReady() {
    ...
    if (window.__TAURI__) {
      await window.__TAURI__.invoke('set_complete', { task: 'frontend' });
    }
  }
  ...
  onMount(async () => {
    ...
    try {
      ...
      console.log('Dashboard - Notification envoyée au backend avec succès');
    } catch (error) {
      console.error('Dashboard - Erreur lors de l\'import ou de l\'exécution de invoke:', error);
    }
  });
</script>
